...
def compile(config):
    ...
    if os.path.exists("init.raw"):
        print(OKGREEN + INFO_PREFIX + "Using init.raw" + ENDC)
        copyfile("init.raw", "%s/init.raw" % config.argument_values["output_dir"])
        download_script += "./hget init.raw init.raw\n"
