...
default_config = {
    "PAYLOAD_SHM_SIZE": 0x20000,
    "PAYLOAD_PREFIX_SHM_SIZE": 0x1000,
    "BITMAP_SHM_SIZE": 0x10000,
    "AGENT_MAX_SIZE": 0x8000000,
    "QEMU_KAFL_LOCATION": "/home/leone/Documents/SGXSan/kAFL/kafl/qemu/x86_64-softmmu/qemu-system-x86_64",
...
def add_args_general(parser):
    ...
    parser.add_argument('-v', help='enable verbose logging to $work_dir/debug.log.',
                        action='store_true', default=False)
    parser.add_argument('-vv', '--debug', help='enable extra debug logging + qemu trace logs in $workdir/.',
                        action='store_true', default=False)
...
def add_args_fuzzer(parser):
    ...
    parser.add_argument('-redq_do_simple', required=False, help='do not ignore simple arith. matches in Redqueen',
                        action='store_true', default=False)
    parser.add_argument('-no_struct_recovery', required=False, help="Disables the struct recovery completely",
                        action='store_true', default=False)
    parser.add_argument('-no_struct_size_detection', required=False, help='skip struct size fields detection',
                        action='store_true', default=False)
    parser.add_argument('-no_struct_ptr_loc_havoc', required=False, help='skip struct pointer location havoc',
                        action='store_true', default=False)
    parser.add_argument('-struct_size_havoc', required=False, help='brute force struct size fields',
                        action='store_true', default=False)
    parser.add_argument('-cpu_affinity', metavar='<n>', help="limit processes to first n cores.",
                        type=int, required=False)
    parser.add_argument('-cpu_affinity_offset', metavar='<n>', help="number to add to the cpu affinity.",
                        type=int, required=False)
...
class UserPrepareConfiguration(six.with_metaclass(Singleton)):
    ...
    def __load_arguments(self):
        ...
        parser.add_argument('-spec', action=FullPath, type=parse_is_dir, help='path to the NYX spec \'build\' folder.')


        parser.add_argument('--afl_mode', help='run afl executable', action='store_true', default=False)
